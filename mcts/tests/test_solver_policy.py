...
def _setup(hand, *, properties: list[tuple[Color, int]] | None = None) -> Game:
...
class MCTSPolicyRegressionTests(unittest.TestCase):
    def test_rollout_and_mcts_charge_birthday_when_legal(self) -> None:
        game = _setup([ItsMyBirthday()], properties=[(Color.RED, 1)])
        rollout_move = choose_move(game)
        mcts_move = ISMCTSSolver(iterations=200).search(game)

        self.assertIsInstance(rollout_move, PlayItsMyBirthday)
        self.assertIsInstance(mcts_move, PlayItsMyBirthday)

    def test_rollout_and_mcts_charge_rent_when_legal(self) -> None:
        game = _setup([RentCard(Color.RED, Color.YELLOW)], properties=[(Color.RED, 2)])
        rollout_move = choose_move(game)
        mcts_move = ISMCTSSolver(iterations=200).search(game)
    ...
    def test_mcts_does_not_bank_birthday_across_seeds(self) -> None:
        for seed in range(10):
            game = _setup([ItsMyBirthday()], properties=[(Color.RED, 1)])
            game._rng = random.Random(seed)
            move = ISMCTSSolver(iterations=200).search(game)
            self.assertNotIsInstance(move, PlayMoneyFromHand, msg=f"seed={seed}")
