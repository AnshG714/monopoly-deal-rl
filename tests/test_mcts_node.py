...
class CommandIdentityKeyTests(unittest.TestCase):
    def test_pay_debt_ignores_money_index_order(self) -> None:
        a = PayDebt([1, 0], [])
        b = PayDebt([0, 1], [])
        self.assertEqual(command_identity_key(a), command_identity_key(b))
...
class ISMCTSNodeIdentityTests(unittest.TestCase):
    ...
    def test_legal_children_match_by_identity_key_not_object_equality(self) -> None:
        node = ISMCTSNode()
    ...
    def test_availability_counts_use_identity_keys(self) -> None:
        node = ISMCTSNode()
        ...
        self.assertEqual(node.availability_counts[command_identity_key(reordered)], 1)
